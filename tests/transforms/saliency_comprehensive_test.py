...
class TestSaliencyMapToBboxesComprehensive:
    ...
    def test_is_small_bbox_width_height_filter(self, transformer_default: SaliencyMapToBboxes):
    ...
    def test_is_small_bbox_invalid_input(self, transformer_default: SaliencyMapToBboxes):
        # Test invalid bbox format
        with pytest.raises(AssertionError, match="bbox must be a list or tuple of 4 integers"):
            transformer_default.is_small_bbox([10, 20, 30])  # Only 3 values
        
        with pytest.raises(AssertionError, match="bbox must be a list or tuple of 4 integers"):
            transformer_default.is_small_bbox([10, 20, 30, 40, 50])  # 5 values

    def test_get_filtered_bboxes_with_filtering(self, transformer_default: SaliencyMapToBboxes):
        # Create mock contours that cv2.boundingRect can handle
        # Large contour - will create bbox [100, 100, 100, 100] (x, y, w, h)
        large_contour = np.array([[[100, 100]], [[200, 100]], [[200, 200]], [[100, 200]]], dtype=np.int32)
        # Small contour - will create bbox [10, 10, 40, 40] which should be filtered out
        small_contour = np.array([[[10, 10]], [[50, 10]], [[50, 50]], [[10, 50]]], dtype=np.int32)
    ...
    def test_get_filtered_bboxes_no_filtering(self, transformer_no_filter: SaliencyMapToBboxes):
        # Create mock contours
        large_contour = np.array([[[100, 100]], [[200, 100]], [[200, 200]], [[100, 200]]])
    ...
    def test_get_filtered_bboxes_sorting(self, transformer_no_filter: SaliencyMapToBboxes):
    ...
    def test_invoke_with_valid_saliency_map(self, transformer_default: SaliencyMapToBboxes):
    ...
    def test_invoke_with_low_threshold_map(self, transformer_default: SaliencyMapToBboxes):
        # Create a saliency map with values below threshold
        img_array = np.full((200, 200), 50, dtype=np.uint8)  # All values below default threshold (100)
        saliency_map = Image.fromarray(img_array, mode='L')
    ...
    def test_invoke_with_rgb_image_error(self, transformer_default: SaliencyMapToBboxes):
        ...
        if len(result) > 1:
            for i in range(len(result) - 1):
                assert (result[i][1] < result[i+1][1] or 
                       (result[i][1] == result[i+1][1] and result[i][0] <= result[i+1][0]))